...
int main(int argc, char const *argv[])
{
	if(argc != 10){
		std::cout << "9 parameters needed and an input file to be specified\n"<<
			"Usage: ./ff_main.out tree_no depthmax threshold randmax randseed gen_no err nw debug  < input_f(x).txt\n"<<
			"example\n"<<
	...
	" randmax "<<randmax<<" randseed "<<randseed<<" gen_no "<<generation_no<<
	" err "<<err<<" fitness "<<E<<" points_no "<<points_no<<"\n";
	delete forest;
	return 0;
}